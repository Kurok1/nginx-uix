...
func TestSPAFallbackServesAssetsFirstAndRejectsUnknownSurfaces(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "POST navigation", method: http.MethodPost, path: "/login", wantStatus: http.StatusNotFound},
		{name: "PUT navigation", method: http.MethodPut, path: "/configuration", wantStatus: http.StatusNotFound},
	}

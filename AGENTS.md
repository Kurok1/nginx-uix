...
- 多步骤功能先写简短实施计划并确认核心边界，不为已经完成的版本维护独立设计文档。
...
- 优先复用 `web/src/styles/` 中的集中 token 和现有组件，不要求先创建独立设计文档。
- 颜色、字体、间距、圆角、阴影和断点应映射为集中设计 token；禁止在业务组件中散落硬编码十六进制颜色。
- 交互主色只有 Action Blue。不得为品牌或装饰引入第二强调色。
- 卡片、按钮和文本不使用装饰阴影。
- 不使用装饰渐变。
- 使用系统字体栈；不得未经许可分发 SF Pro 字体文件。
- 触控目标不得小于 44×44px；键盘焦点必须清晰可见。
- 响应式实现复用现有断点与折叠方式，并验证桌面、平板和手机，不只适配桌面宽度。
- 表格、树、编辑器、diff、Modal、Toast、状态徽标和错误态优先复用现有组件；新增模式保持接口小而直接。
- 成功、警告、错误等语义状态不能只依赖颜色；新增语义色集中到 token，并保持其只用于状态而非品牌强调。
...
- 现有认证、输入校验和权限行为继续保留，但不作为当前版本发布阻断项。
- 当前 CI 不引入漏洞扫描、SBOM、安全审计或权限验证等第三方门禁。
- 发现安全问题后作为独立缺陷修复，不扩大当前发布流程。
...
- linux/amd64 与 linux/arm64 二进制和镜像能够构建。
- 文档或机械变更通过链接、语法或差异检查。
...
- 新增或改变视觉 token、组件和交互规则时同步更新对应源码与行为测试。
